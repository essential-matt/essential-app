...
final class FeedAcceptanceTests: XCTestCase {
    func test_onLaunch_displaysRemoteFeedWhenCustomerHasConnectivity() {
        ...
        XCTAssertEqual(feed.renderedFeedImageData(at: .zero), makeImageData())
    ...
    func test_onLaunch_displaysCachedRemoteFeedWhenCustomerHasNoConnectivity() {
        ...
        XCTAssertEqual(offlineFeed.renderedFeedImageData(at: .zero), makeImageData())
        XCTAssertEqual(offlineFeed.renderedFeedImageData(at: 1), makeImageData())
    }

    func test_onLaunch_displaysEmptyFeedWhenCustomerHasNoConnectityAndNoCache() {
        let feed = launch(httpClient: .offline, store: .empty)

        XCTAssertEqual(feed.numberOfRenderedFeedImageViews, .zero)
    }
